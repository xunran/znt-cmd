...
| K-04 canary routing/hits | default `agent.run` can route to canary release by canary percent and records `canary.routed` trace with `strategy_hash`, `manifest_hash`, `source_kind`, `carrier_kind`, and `runtime_contract` release evidence | `internal/server.TestPackageCanaryRoutesDefaultTrafficAndRecordsHit` |
