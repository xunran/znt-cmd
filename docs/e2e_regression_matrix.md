...
| K-04 canary routing/hits | default `agent.run` stays on the configured default version even when a canary release exists; canary trace constants remain registered for compatibility | `internal/server.TestPackageCanaryDoesNotOverrideDefaultRunVersion`, `canary.routed` |
